...
public:
  ...
  void SelectTransmissionStrategy (uint8_t  SelectStrategy);

    void SelectBDPFrequency(uint8_t BDPF);
private:
  ...
  uint8_t m_pktSize; // the packet size.
    ...
    uint8_t EnableTransmissionStrategy = 0;
    uint8_t EnableBiDirectionalProcedure = 0;
