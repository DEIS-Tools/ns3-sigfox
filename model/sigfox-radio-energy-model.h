...
#define SIGFOX_RADIO_ENERGY_MODEL_H

#include "ns3/device-energy-model.h"
#include "ns3/traced-value.h"
#include "end-point-sigfox-phy.h"
 ...
 * \ingroup energy
 */
class SigfoxRadioEnergyModelPhyListener : public EndPointSigfoxPhyListener
{
public:
  /**
  ...
  virtual ~SigfoxRadioEnergyModelPhyListener ();
  ...
  /**
   * \brief Switches the SigfoxRadioEnergyModel to RX state.
   *
   * \param duration the expected duration of the packet reception.
   *
   * Defined in ns3::SigfoxEndPointPhyListener
   */
  void NotifyRxStart (void);

  /**
   * \brief Switches the SigfoxRadioEnergyModel to TX state and switches back to
...
private:
  ...
  /**
   * Change state callback used to notify the SigfoxRadioEnergyModel of a state
   * change.
   */
  DeviceEnergyModel::ChangeStateCallback m_changeStateCallback;

  /**
   * Callback used to update the tx current stored in SigfoxRadioEnergyModel based on
 ...
 *
 */
class SigfoxRadioEnergyModel : public DeviceEnergyModel
{
public:
  /**
   * Callback type for energy depletion handling.
   */
  typedef Callback<void> SigfoxRadioEnergyDepletionCallback;

  /**
   * Callback type for energy recharged handling.
   */
  typedef Callback<void> SigfoxRadioEnergyRechargedCallback;
  ...
  SigfoxRadioEnergyModel ();
  virtual ~SigfoxRadioEnergyModel ();
   ...
   *
   * \returns idle current of the sigfox device.
   */
   ...
   *
   * \returns transmit current of the sigfox device.
   */
   ...
   *
   * \returns receive current of the sigfox device.
   */
   ...
   *
   * \returns sleep current of the sigfox device.
   */
   ...
   */
  EndPointSigfoxPhy::State GetCurrentState (void) const;
   ...
   */
  void SetEnergyDepletionCallback (SigfoxRadioEnergyDepletionCallback callback);
   ...
   */
  void SetEnergyRechargedCallback (SigfoxRadioEnergyRechargedCallback callback);

  /**
   * \param model the model used to compute the sigfox tx current.
   */
  // NOTICE VERY WELL: Current  Model linear or constant as possible choices
  void SetTxCurrentModel (Ptr<SigfoxTxCurrentModel> model);
  ...
  /**
   * \brief Changes state of the SigfoxRadioEnergyMode.
   *
   * \param newState New state the sigfox radio is in.
   *
   ...
   */
  SigfoxRadioEnergyModelPhyListener *GetPhyListener (void);
...
private:
   ...
   */
  void SetSigfoxRadioState (const EndPointSigfoxPhy::State state);
  ...
  // NOTICE VERY WELL: Current  Model linear or constant as possible choices
  Ptr<SigfoxTxCurrentModel> m_txCurrentModel; ///< current model

  /// This variable keeps track of the total energy consumed by this model.
  TracedValue<double> m_totalEnergyConsumption;
  TracedValue<double> m_systemcurrent = 0.0035;
  // State variables.
  EndPointSigfoxPhy::State m_currentState; ///< current state the radio is in
  ...
  SigfoxRadioEnergyDepletionCallback m_energyDepletionCallback;

  /// Energy recharged callback
  SigfoxRadioEnergyRechargedCallback m_energyRechargedCallback;

  /// EndPointSigfoxPhy listener
  SigfoxRadioEnergyModelPhyListener *m_listener;
};

} // namespace sigfox

} // namespace ns3
#endif /* SIGFOX_RADIO_ENERGY_MODEL_H */
