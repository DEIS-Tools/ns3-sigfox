  ...
  for (uint32_t i = 0; i < node->GetNDevices (); i++)
    {
      ...
      if (currentNetDevice->GetObject<SigfoxNetDevice> () != 0)
        {
          Ptr<SigfoxNetDevice> sigfoxNetDevice =
            currentNetDevice->GetObject<SigfoxNetDevice> ();
          app->SetSigfoxNetDevice (sigfoxNetDevice);
          sigfoxNetDevice->SetReceiveCallback (MakeCallback
                                               (&Forwarder::ReceiveFromSigfox, app));
        }
        ...
        {
          NS_LOG_ERROR ("Potential error: NetDevice is neither Sigfox nor PointToPoint");
        }
    }
