 ...
 */
struct SigfoxTxParameters
{
  double bandwidthHz = 100;     //!< Bandwidth in Hz
  uint32_t nPreamble = 8;     //!< Number of preamble symbols
};

/**
 * Allow logging of SigfoxTxParameters like with any other data type.
 */
std::ostream &operator << (std::ostream &os, const SigfoxTxParameters &params);

/**
 * \ingroup sigfox
 *
 * Base class for PHY layers implementing the Sigfox modulation scheme.
 *
 ...
 * SigfoxTxParameters objects.
 */
class SigfoxPhy : public Object
{
public:
  // TypeId
   ...
   *
   * This method is typically called by SigfoxChannel.
   *
   ...
   * passing a SigfoxInterferenceHelper Event to this method, the class will be
  ...
  virtual void Send (Ptr<Packet> packet, SigfoxTxParameters txParams,
                     double frequencyHz, double txPowerDbm) = 0;
  ...
  /**
   * Set the SigfoxChannel instance PHY transmits on.
   *
   * Typically, there is only one instance per simulation.
   *
   * \param channel The SigfoxChannel instance this PHY will transmit on.
   */
   ...
   *
   * \return The SigfoxChannel instance this PHY transmits on.
   */
  Ptr<SigfoxChannel> GetChannel (void) const;
   ...
   */
  static Time GetOnAirTime (Ptr<Packet> packet, SigfoxTxParameters txParams);
...
protected:
  ...
  Ptr<SigfoxChannel> m_channel; //!< The channel this PHY transmits on.

  SigfoxInterferenceHelper m_interference; //!< The SigfoxInterferenceHelper
