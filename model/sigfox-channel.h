...
#ifndef SIGFOX_CHANNEL_H
#define SIGFOX_CHANNEL_H
 ...
 * SigfoxChannel.
 */
struct SigfoxChannelParameters
{
...
/**
 * Allow logging of SigfoxChannelParameters like with any other data type.
 */
 ...
 */
class SigfoxChannel : public Channel
{
public:
  // TypeId
  ...
  /**
    * Construct a SigfoxChannel with a loss and delay model.
    *
    * \param loss The loss model to associate to this channel.
    * \param delay The delay model to associate to this channel.
    */
  SigfoxChannel (Ptr<PropagationLossModel> loss,
               Ptr<PropagationDelayModel> delay);

  /**
    * Connect a SigfoxPhy object to the SigfoxChannel.
    *
    ...
    */
  void Add (Ptr<SigfoxPhy> phy);

  /**
    * Remove a physical layer from the SigfoxChannel.
    *
  ...
  /**
    ...
    */
  void Send (Ptr<SigfoxPhy> sender, Ptr<Packet> packet, double txPowerDbm,
             SigfoxTxParameters txParams, Time duration, double frequencyMHz)
  const;
...
private:
  /**
    * Private method that is scheduled by SigfoxChannel's Send method to happen
  ...
  /**
    ...
    * channel.
    */
  std::vector<Ptr<SigfoxPhy> > m_phyList;
